...
logger.info("Initializing the CrossEncoder model...")
...
class SparseVectorStore(BaseVectorStore):
    ...
    def _calculate_idf(self, doc_count: int, corpus_size: int) -> float:
        ...
        idf_score = np.log((corpus_size - doc_count + 0.5) / (doc_count + 0.5) + 1.0)
        return idf_score
        ...
        for q in tokenized_query:
            if q in self.idf:
                for doc_id, doc_freqs in enumerate(self.doc_freqs):
                    ...
                    idf = self.idf[q]
                    score[doc_id] += idf * (f * (self.k1 + 1)) / (f + self.k1 * (1 - self.b + self.b * doc_length / self.avgdl))
    ...
    def query(self, query: str, top_k: int = 3) -> VectorStoreQueryResult:
        ...
        initial_nodes = [self.node_list[node_id] for node_id in best_ids]

        # Re-rank using cross-encoder
        query_pairs = [[query, node.text] for node in initial_nodes]
        logger.info(f"Cross-encoder input pairs: {query_pairs}")
        cross_encoder_scores = CROSS_ENCODER_MODEL.predict(query_pairs)
        logger.info(f"Cross-encoder scores: {cross_encoder_scores}")
        
        re_ranked_indices = np.argsort(cross_encoder_scores)[::-1]
        re_ranked_nodes = [initial_nodes[idx] for idx in re_ranked_indices]
        re_ranked_scores = [cross_encoder_scores[idx] for idx in re_ranked_indices]

        return VectorStoreQueryResult(
            nodes=re_ranked_nodes,
            similarities=re_ranked_scores,
            ids=[node.id_ for node in re_ranked_nodes],
        )
    ...
    def batch_query(
        ...
        Returns:
            List[VectorStoreQueryResult]: List of query results.
        """
